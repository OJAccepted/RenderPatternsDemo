...
import { ssr, otherSSR } from "./build/ssr/main.js";
...
app.get("/api/users/:user", (req, res) => {
	const user = DATA.filter(
		(user) =>
			user.login.uuid === req.params.user ||
			user.login.username === req.params.user
	)[0];
...
app.get("/streaming", async (request, response) => {
	try {
		...
		const newStream = await otherSSR({
			url: request.url,
			streaming: true,
		});
		...
		await new Promise((resolve, reject) => {
			...
			stream.on("end", () => {
				setTimeout(() => {
					newStream.pipe(response, { end: false });
					resolve();
